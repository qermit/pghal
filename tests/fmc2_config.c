...
int main( int argc, char** argv){
  ...
  struct pghal_bus * bus = xdma_open_bus("/dev/xdma/card4/user");

//  struct wb_gpio_raw * fmc1_gpio = wb_gpio_raw_create_direct(bus, OFFSET_FMC1 | OFFSET_DIO5_GPIO);
  struct wb_gpio_raw * fmc2_gpio = wb_gpio_raw_create_direct(bus, OFFSET_FMC | OFFSET_ADC250_GPIO);
...
//  wb_gpio_raw_set_port_termination(fmc1_gpio, 0b00000001); 
  ...
  double fnew = 100.0;
  struct wb_i2c * si57x_i2c_bus = wb_i2c_init(bus , OFFSET_FMC | OFFSET_ADC250_I2C_SI57X);
...
//  wb_gpio_raw_set_port_altf(fmc2_gpio, 0x8); 
  ...
  if (1) {
  // configure AD9510
  struct wb_spi * spi_ad9510   = wb_spi_init(bus, OFFSET_FMC | OFFSET_ADC250_SPI_AD9510);
...
//  return 0;
  ...
  //configure 4x ADC
  struct wb_spi * spi_isla216p = wb_spi_init(bus, OFFSET_FMC | OFFSET_ADC250_SPI_ISLA);
  if (1) {
  ...
  return 0;
  struct wb_spi * spi_amc7823  = wb_spi_init(bus, 0x00004040);
  ...
  uint32_t addr = 0x200;
//@todo  printf("xdma_read32 : 0x%08X <- 0x%08X\n", addr, xdma_read_direct(xdma, addr));
  printf("method xdma_write_direct\n");
  int i;
  for(i = 0; i<10; i++) {
//@todo    xdma_write_direct(xdma, 0x00004204, 1 << 3 | 1);
    usleep(1000 * 100);
//@todo    xdma_write_direct(xdma, 0x00004200, 1 << 3 | 1);
    usleep(1000 * 100);
  }
  usleep(1000 * 1000);
  printf("method pghal_bus_write\n");
  struct sdb_node_address * sdb_addr_set = sdb_address_create(bus, 0x4204);
  struct sdb_node_address * sdb_addr_clr = sdb_address_create(bus, 0x4200);
  uint32_t data[1] = { 1<<3 | 2 };
  for(i = 0; i<10; i++) {
    pghal_bus_write(bus, &sdb_addr_set->address, 1*sizeof(uint32_t), data);
    usleep(1000 * 100);
    pghal_bus_write(bus, &sdb_addr_clr->address, 1*sizeof(uint32_t), data);
    usleep(1000 * 100);
  }
