...
int init_adc(struct fmc_adc250m * fmc_card, double fnew) {
...
//  return 0;
  ...
  { int i;
    ...
    for (i = 0 ; i<isla_chips; i++){
      ...
      fmc_card->chip_isla216p[i]->reg_current.out[3] = 0x24; // binary offset
      fmc_card->chip_isla216p[i]->reg_current.out[3] = 0x20; // two complement
      if (fnew < 90) {
