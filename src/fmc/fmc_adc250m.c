...
struct fmc_adc250m * fmc_adc250m_init(struct pghal_bus * bus, uint32_t bus_address, uint32_t sdb_address)
{
  ...
  len = wb_sdb_get_addr_by_id(fmc->sdb, C_SPI_MON, &fmc->address[C_SPI_MON]);
  len = wb_sdb_get_addr_by_id(fmc->sdb, C_GPIO, &fmc->address[C_GPIO]);
  if (len >= 0) 
    fmc->gpio = wb_gpio_raw_create_direct(bus, fmc->address[C_GPIO]);

  len = wb_sdb_get_addr_by_id(fmc->sdb, C_IDELAY, &fmc->address[C_IDELAY]);
